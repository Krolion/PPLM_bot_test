...
def top_filtering(logits, top_k=0., top_p=0.9, threshold=-float('Inf'), filter_value=-float('Inf')):
    """ Filter a distribution of logits using top-k, top-p (nucleus) and/or threshold filtering
        Args:
            ...
            filter_value: the fuck do i know? probably something important
    """
...
def run():
    ...
    parser.add_argument("--top_k", type=int, default=10, help="Filter top-k tokens before sampling (<=0: no filtering)")
    ...
    while True:
        ...
        history.append(pert_gen_tok_texts[0].tolist()[0])
        history = history[-(2*args.max_history+1):]
        out_text = tokenizer.decode(pert_gen_tok_texts[0].tolist()[0], skip_special_tokens=True)
